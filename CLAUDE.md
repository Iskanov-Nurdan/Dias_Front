...
`BACKEND_SPEC.md` в корне — **устаревший** документ времён старого проекта
(описывает вымышленный бэкенд Rahman Ata на `rahmanata.kg`). Он не имеет
отношения к DIAS_ERP, не используй его как источник правды по API.
