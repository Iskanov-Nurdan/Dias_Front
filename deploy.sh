...
ssh "$SERVER" "mkdir -p '$REMOTE_DIR' && rm -rf '$REMOTE_DIR'/* && mv '$REMOTE_DIR.new'/* '$REMOTE_DIR'/ && rmdir '$REMOTE_DIR.new' && chmod -R a+rX '$REMOTE_DIR'"
# scp по SFTP иногда кладёт файлы без права на чтение для других — без этого
# nginx (www-data) отдаёt 404 на все /static/*, хотя файлы реально на месте.
