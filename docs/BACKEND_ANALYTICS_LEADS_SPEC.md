...
После внедрения эндпоинта фронт будет:

1. Вызывать `GET /api/analytics/leads/?year=2025&month=2` вместо `GET /api/leads/?perPage=2000`
2. Использовать готовые агрегаты вместо клиентской обработки
3. Продолжать загружать `GET /api/leads/funnel-stages/` и `GET /api/sports/` для справочников (если нужны названия этапов/спортов вне агрегатов)
