# Задача для бэкенда: Operational WebSocket (DIAS)

Скопируйте этот документ в задачу/PR бэкенда. Фронт **уже реализован** под этот контракт; без событий с бэка разделы не обновляются в реальном времени.
