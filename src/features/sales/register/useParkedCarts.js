...
const DEFAULT_STORAGE_KEY = 'dias_pos_parked_carts_v1';
...
const writeAll = (STORAGE_KEY, list) => {
  try {
...
/**
 * Отложенные чеки кассы — намеренно НЕ на бэкенде. Чекаут в apps.sales/apps.foam —
 ...
 * цены) перед оплатой — это ответственность вызывающей модалки, не этого хука.
 *
 * storageKey — отдельный слот на кассу: у Профиля и Foam разная форма строк
 * чека (profileId vs stockId), общий список их бы перемешал.
 */
export function useParkedCarts(storageKey = DEFAULT_STORAGE_KEY) {
  const [carts, setCarts] = useState(() => readAll(storageKey));

  useEffect(() => {
    setCarts(readAll(storageKey));
    const onStorage = (e) => {
      if (e.key === storageKey) setCarts(readAll(storageKey));
    };
  ...
  const remove = useCallback((id) => {
    const next = readAll(storageKey).filter((c) => c.id !== id);
    writeAll(storageKey, next);
    setCarts(next);
  }, [storageKey]);
